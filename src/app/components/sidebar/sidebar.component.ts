...
export class SidebarComponent implements AfterViewChecked, AfterViewInit{
  ...
  constructor(private eRef: ElementRef, private usuarioService:UsuarioService,private router:Router) {
    if (window.innerWidth <= 767) {
      this.mobileMode = true;
      this.desktopMode = false;
    } else {
    ...
    this.usuarioService.user.subscribe(x => {
          this.user = x;
          // this.usuarioService.getPermisosByIdRol();
          this.getValidName();
    });
    ...
    if(!menu){
      ...
      this.usuarioService.menuActual$.subscribe((menuA:any)=>{
        if(menuA != undefined && Array.isArray(menuA)){
         console.log('constructor sidebar',this.menus);
         this.menus = menuA;
        }
      });
    }
